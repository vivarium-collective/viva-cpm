...
| D8 | `init_viral_load` IC over-seeds ~25× | source `v0` = total virus PER EPITHELIAL CELL, `v0_sites = v0·num_epi/(dim.x·dim.y) ≈ v0/cell_sites` per pixel (source line 92-93) | `_seed_uniform_virus` deposits `load` per PIXEL = `load·cell_sites` per cell → 25× the source | missing `/cell_sites` (=cell_volume), the SAME factor as D2's g_vh | 🟡 **root confirmed; fix needs #10 3-way reconciliation** | Confirmed: running our loads ÷25 restores the fig5 dose-response (effective v0=0.04 → 82% survive, v0≥4 → total loss). This is the dominant residual D2 cause. BUT a self-consistent fix must reconcile the IC (`load/cell_sites`), the observable mapping (`_map_full_model_observables` divides field sum by tissue sites so virus t0 currently = `load`), AND the fig5.json target convention together — the #10 units across 3 quantities, a real increment. |
