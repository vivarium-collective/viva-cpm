...
lives under `viva_cpm_studies/visualizations/` (owned by a peer session); do
...
def ifn_resistance_figure(with_result: dict, without_result: dict) -> Figure:
    ...
    `viva_cpm_studies/visualizations/` (owned by a peer session). Returns a
...
def repro_fig3b_figure(result: dict) -> Figure:
    ...
    This is a minimal in-package stub, NOT the polished viz system under
    `viva_cpm_studies/visualizations/` (owned by a peer session; see
