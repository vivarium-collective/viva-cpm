...
Unlike ``viva_cpm_studies.chemotaxis.run`` (which uses raw ``cpm.load_world``
...
underlying occupancy signal. See ``viva_cpm_studies.chemotaxis.metrics.
recruitment_index_all_responders``.

Run (from the worktree root, using the viva-cpm venv)::

    PYTHONPATH=. python -c "from viva_cpm_studies.chemotaxis.run_receptor import run_receptor; \\
